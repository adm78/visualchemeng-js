function CollisionStats() {
    ...
    this.collision_frequency = function (event_log, volume, particle_list, config) {
        ...
        for (let i = 0; i < event_log.n_events(); i++) {
            let event = event_log.event(i);
            if (!event.is_wall_collision()) {
                let coll_pair_small_part = event.smallest_particle();
                let coll_pair_large_part = event.largest_particle();
                let x_sieve_index = this._get_sieve_index(coll_pair_small_part.radius, sieve_sizes);
                let y_sieve_index = this._get_sieve_index(coll_pair_large_part.radius, sieve_sizes);
                n_coll[x_sieve_index][y_sieve_index] += 1;
                n_coll[y_sieve_index][x_sieve_index] += 1;
            }
        }
    ...
    this._get_sieve_index = function (value, sieve_list) {
        ...
        for (let sieve_index = 0; sieve_index < sieve_list.length - 1; sieve_index++) {
            if (sieve_list[sieve_index + 1] > value && value >= sieve_list[sieve_index]) {
                return sieve_index;
            }
        }
