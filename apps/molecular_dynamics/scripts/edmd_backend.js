...
function EDMDSimulation(canvas, config) {
    ...
    this._initialise_particles = function () {
        ...
        this.particles = [];
        let dx = this._get_initial_spacing();
        let n_init = 0;
        let potential_part;
        for (var i = 0; i < Math.round(this._canvas.width / dx); i++) {
            ...
            for (var j = 0; j < Math.round(this._canvas.height / dx); j++) {
                if (n_try < this.config.n_init_try_max) {
                    let particle_options = {radius: this._get_particle_radius()};
    ...
    this._get_new_collision_list = function () {
        ...
        for (i = 0; i < this.particles.length; i++) {
            ...
            first_event = this._get_wall_collision_event(i);
    ...
    this._get_wall_collision_event = function (part_index) {
        ...
        // locals vars
        let t_side; // side wall collision time
        let t_ud;   // top or bottom wall collision time
        let w_side; // which side wall ('r' or 'l')
        let w_ud;   // top or bottom wall first? ('u' ,d')
        let part = this.particles[part_index];
    ...
    this._overlap_exists_particle = function (p1, p2) {
        ...
        return this._dist(p1, p2) < p1.radius + p2.radius;
    };
    ...
    this._particle_in_sim_box = function (part) {
        ...
        return 0 < part.pos.x - part.radius
