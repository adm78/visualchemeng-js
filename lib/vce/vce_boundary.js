...
function Boundary(x, y, w, h, a, world, mode=CENTER) {
    ...
    */
	...
	// var point = this.center2left(this.body.position.x, this.body.position.y, this.w, this.h, this.body.angle);
	// //var scaleX = x/point.x;
	// //var scaleY = (y)/point.y;
	// var scaleX = Math.abs(x-point.x)/point.x;
	// var scaleY = Math.abs(y-point.y)/point.y;
	// console.log("scaleX = ", scaleX, "scaleY = ", scaleY);
	// Body.scale(this.body, scaleX, scaleY, point);
    };
	...
	if (this.active) {
	    if (this.is_on_corner(mouseX, mouseY)) { this.scale(mouseX, mouseY); }
	    else {this.translate(mouseX, mouseY); };
	};
    };


    this.keyPressed = function() {
	if (this.active) { this.rotate(keyCode); };
    };
