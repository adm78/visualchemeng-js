...
function PhysEngineParticle(world, x, y, r, colour, shape, options) {

    // This particle is compatible with both the matter.js
    // physics engine and the p5 rendering library.='#2e8ade'

    // set some defaults
    if (typeof shape == 'undefined') { shape = {'type':'circle'}};
    if (typeof colour == 'undefined') { colour = '#2e8ade'};
    if (typeof options == 'undefined') {
	options = {friction: 0, restitution: 0.95};
    };
	...
	this.body = Bodies.polygon(x, y, this.shape['sides'], r, options);
    }
    else {
