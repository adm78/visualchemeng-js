// VCE Project - Simple particle related classes
//
// Contains:
// Particle - A simple class to store particle information and apply
// various transformations to simulate particle movement.
//
// PhysEngineParticle - A more function-rich particle class that has
// been integrated with the matter.js physics engine. Mirrors the
// interface of the simpler Particle class where applicable.
//
// ParticleFeed - A data structure for storing information about a
// particle source.
// 
//
// Requires:
// - p5.js or p5.min.js
// - matter.js or matter.min.js (for PhysEngineParticle only)
// - vce_utils.js
//
...
function PhysEngineParticle(world, x, y, options, force) {
	...
	shape  : {type:'circle'},
	colour : '#2e8ade',
	matter_options : {friction: 0, restitution: 0.95}
    };
    var options = merge_options(default_options, options);
    var default_force = { x : 0, y : 0 };
    var force = merge_options(default_force, force);
	...
	if (this.shape.type == 'circle') {
	    ...
	    polygon(0, 0, this.radius, this.shape.sides);
	};
	pop();
    }
...
function ParticleFeed(x, y, rate, particle_options, force = {x:0, y:0}) {
    /* A particle feed class compatible with p5.js and
       ...
       Optional:
           - particle_options (JSON object) : Specifies the graphical
             properties of the particle. See
             vce_multibody.ParticleFactory for more details on this
             optinal arg.
	   - force (JSON vector) : Specifies to force to apply to each
             feed particle at the moment they are created.
    */
