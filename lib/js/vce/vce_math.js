...
var vce_math = {
    ...
    zeros_2d: function (shape) {
    	// I'm sure I'll be motivated enough to right the n-dimensional version at some point...
        let my_array = [];
        for (let i = 0; i < shape[0]; i++) {
            my_array.push(this.zeros_1d(shape[1]))
        }
        return my_array;
    },
    ...
    interp_1d: function (x, y, extrapolate = false) {
        ...
        var f = function (x_i) {
            ...
            if (x_i == null || x_i == 'undefined') {
                ...
                if (extrapolate) {
                    if (x_i > x[end]) {
                        var end = x.length - 1;
                        var m = (y[end] - y[end - 1]) / (x[end] - x[end - 1]);
                        var c = y[end] - m * x[end];
                    } else {
