...
function Component() {
    ...
    this.rho_l_eqn = 1;
    this.rho_l = function(T) { // (kg/m3)
	if (this.rho_l_eqn === 1) {
	    var num = this.A_rho_l;
	    var den_exp = 1 + (1 - Math.pow(T/this.C_rho_l, this.D_rho_l))
	    var den = Math.pow(this.B_rho_l, den_exp);
	    return num*this.awm/den; 
	} else if (this.rho_l_eqn === 2) {
	    return 1000.0*(this.A_rho_l + this.B_rho_l*T + this.C_rho_l*Math.pow(T,2) + this.D_rho_l*Math.pow(T,3))*this.amw;
	} else {
	...
	return Math.exp(this.A_mu_l + (this.B_mu_l/T) + this.C_mu_l*Math.log(T) + this.D_mu_l*Math.pow(T, this.E_mu_l));
    };
