...
function Ensemble(p=[],world=null) {
    ...
    */
    ...
    this.update = function(xmax, ymax, dx_max, dy_max, gravity) {
	...
	this.removeOutliers(xmax,ymax);
	this.updateOutliers();
	this.updateFeeds();
	this.perturb(dx_max, dy_max);
	this.apply_buoyant_force(gravity);
    };
	...
	for (var i=0; i < this.feeds.length; i++) {
	    ...
	    for (var j=0; j < n1; j++) {
		if (this.engine_mode) {
		    var part = ParticleFactory(this.world, feed.x, feed.y, feed.particle_options, feed.force);
		}
		else {
