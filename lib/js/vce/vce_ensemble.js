...
function Ensemble(p=[],world=null,name='',) {
    ...
    */
	...
	Parameters
	----------
	source_args : array of array of float or array of float or null
	...
	Notes
	-----
	If only one particle source exists, then source_args can
	just be a array of floats.  If source_args is null, null is
	passed to get_particle_options_weighted for each stream.
	...
	*/
	...
	for (var i=0; i < this.sources.length; i++) {
		...
		if (utils.isArray(source_args[i])) {
		    ...
		    var current_source_args = source_args; // there's only one source, accept un-nested soure_args
		} else {
		    throw new Error("source_args in not an array type, but more than one source arg is associated with the current ensemble. Check that your particle options are correctly formed.")
		}
	    };
