...
function Ensemble(p=[],world=null,name='',) {
    ...
    */
	...
	// this does not update the physic engine, since multiple
	// ensembles may be associated with the same engine. This
	// function is generally not utilised. Instead the relevant
	// sub-calls are made manually at regular intervals.
	// options attributes: xmax, ymax dx_max, dy_max, gravity
	this.removeOutliers(options.xmax, options.ymax);
	this.updateOutliers();
	this.updateFeeds(options.feed_args);
    ...
    this.updateFeeds = function(feed_args) {
	...
	for (var i=0; i < this.feeds.length; i++) {
	    ...
	    if (feed_args == null) {
		var current_feed_args = null;
	    }
	    else {
		var current_feed_args = feed_args[i]; 
	    };
	    var p_options = feed.get_particle_options_weighted(current_feed_args);
	    for (var j=0; j < n1; j++) {
		var part = ParticleFactory(this.world, feed.x, feed.y, p_options);
