...
function Valve(x, y, options) {
	...
	if (flow_capacity <= 1.0 && flow_capacity >= 0.0) {
	    throw new RangeError("flow capcity out of range 0 <= fc <= 1.0. Got flow_capacity =", flow_capacity);
	};
	...
	if (this._supported_types.contains(type)) {
	    this._type = type;
	} else {
    ...
    this._set_position_from_flow_capacity_equal_percentage(fc) {
	this.positon = Math.log(Fc*(Math.exp(1) - 1.0) + 1.0);
    };
	...
	return createVector(this.x - 0.007*this.images.body.width*this._scaling,
			    this.y + 0.15*this.images.body.height*this._scaling)
    };
	...
	var x_pos = this.x;
	var y_pos = this.y + 0.9*this.images.body.height*this._scaling;
